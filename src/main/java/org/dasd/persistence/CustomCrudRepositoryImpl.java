...
import org.dasd.domain.QWebReply;
    ...
    public Page<Object[]> getCustomPage(String type, String keyword, Pageable page) {
        ...
        if(type!=null){
            switch (type.toLowerCase()){
                case "t":tuple.where(b.title.like("%"+keyword+"%")); break;

                case "c":tuple.where(b.content.like("%"+keyword+"%")); break;

                case "w":tuple.where(b.writer.like("%"+keyword+"%")); break;
            }
        }
